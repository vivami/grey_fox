    ...
    if (strcmp("SYS_open", syscall) == 0) {
        ...
        if (!error) {
            if (strstr(path, "/.") != NULL) {
                ...
                kprintf("[GREY FOX] %lu:%lu:%lu,%u; %s; %d; %d; %s; %d; %s;\n",
                       hours,
                       mins,
                       secs,
                       microsecs,
...
//        struct bsdthread_create_args* args = (struct bsdthread_create_args*) a;
//        printf("[GREY FOX] %lu:%lu:%lu,%u; %s; %d; %d; %s; %d; %d;\n",
//               hours,
//               mins,
//               secs,
//               microsecs,
...
//               );
        ...
        kprintf("[GREY FOX] %lu:%lu:%lu,%u; %s; %d; %d; %s; %d;\n",
               hours,
               mins,
               secs,
               microsecs,
